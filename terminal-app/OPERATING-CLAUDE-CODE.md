...
**What happened:** During ftdi-webusb-driver Phase 10, `typedoc-plugin-markdown`
v4+ was run with `"out": "."` in `typedoc.json`. The plugin cleans its output
directory before generating docs. With the output directory set to the project
root, it deleted the entire working tree — `src/`, `.git/`, `node_modules/`,
